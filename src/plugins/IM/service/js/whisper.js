...
$(function () {
    ...
    // 结束会诊
    $('#end-ask').click(function () {
        var layerIndex = null;
        layerIndex = layer.confirm('确定要结束会诊吗?', {
            title: '结束提示',
            closeBtn: 0,
            icon: 3,
            btn: ['确定', '取消'],
...
function sendMessage(sendMsg) {
    ...
    socket.send(JSON.stringify({
        type: 'chatMessage',
        data: {
            to_id: uid, to_name: uname, content: msg, from_name: uinfo.username,
            from_id: uinfo.id, from_avatar: uinfo.avatar, ask_order_id: 1,
        },
    }));
